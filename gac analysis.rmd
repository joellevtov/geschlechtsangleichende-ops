...
if (Sys.info()["sysname"] == "Darwin") {  # Darwin is the OS name for Mac
    setwd("/Users/joellevtov/Library/CloudStorage/OneDrive-SWMHGmbH/GAC")
...
gac_data <- read.csv2('/Users/joellevtov/Library/CloudStorage/OneDrive-SWMHGmbH/GAC/GAC Daten Roh.csv')
gac_data <- gac_data |> 
    mutate(Anzahl = as.numeric(Anzahl))
# Xs become NAs when converting to numeric

# Die Rohen Zahlen: Wie viele Leute kriegen überhaupt solche OPs in Deutschland?
gac_data_total_per_year <- gac_data |> 
    ...
    view()
gac_data_total_per_year <- gac_data_total_per_year |>
    pivot_wider(
        names_from = Geschlecht,
        values_from = Anzahl
    ) |>
    view()
write_csv(gac_data_total_per_year, "gac_data_total_per_year.csv")

# * Analysis by Wohnort (residence) *
by_wohnort <- gac_data |> 
    filter(Wohnort.Behandlungsort=="nach Wohnort")
by_wohnort <- gac_data |> 
    filter(Wohnort.Behandlungsort == "nach Wohnort") |> 
...
# Für Histogramm - durchschnittliches Alter der Person, die die OP kriegt
...
# Nur Deutschland insgesamt
age_distribution_germany_only <- gac_data |> 
    filter(Bundesland == "Deutschland") |> 
    group_by(Altersgruppe) |> 
    filter(Altersgruppe != "Alle Altersgruppen" &
           Altersgruppe != "Unbekannt") |>
    summarize(mean_num_surgeries_per_year = mean(Anzahl, na.rm = TRUE)) |>
...
view(age_distribution_germany_only)
...
print("Das heißt, es gab 6 Fälle von GAC Operationen bei Kindern unter 5 Jahren in ganz Deutschland von 2005-2024.")
write_csv(unter_5_Jahren_gac_surgeries, "unter_5_Jahren_gac_surgeries.csv")

# Pro Bundesland
by_wohnort_bundesland_average <- by_wohnort |>
    group_by(Bundesland) |> 
    summarize(mean_anzahl = mean(Anzahl, na.rm = TRUE)) 
print(by_wohnort_bundesland_average) # not yet per 100k people - don't read too much into this

if (Sys.info()["sysname"] == "Darwin") {  # Darwin is the OS name for Mac
    population <- read_csv("/Users/joellevtov/Library/CloudStorage/OneDrive-SWMHGmbH/pop bundesländer.csv")
} else if (Sys.info()["sysname"] == "Windows") {
...
bundesländer_avg_gac_surgeries <- bundesländer_avg_gac_surgeries |> 
    mutate(surgeries_per_100k = (mean_anzahl / Bevölkerung) * 100000) |> 
    arrange(desc(surgeries_per_100k))
print(bundesländer_avg_gac_surgeries)

bundesländer_avg_gac_surgeries <- bundesländer_avg_gac_surgeries |> 
    select(-mean_anzahl, -Bevölkerung) |> 
